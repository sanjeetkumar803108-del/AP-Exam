...
Response JSON Schema:
{
  ...
  "howToGetFullPoints": [
    ...
    const isQuestionPrompt = !isMCQ && (parsed.submissionMode === "question_prompt" || parsed.questionType === "subjective_frq_question" || parsed.detectedContentType === "printed_frq_question" || parsed.verificationVerdict === "GENUINE_EXAM_QUESTION");
    const isStudentAnswer = !isMCQ && (parsed.submissionMode === "student_answer" || parsed.submissionMode === "question_and_answer" || parsed.questionType === "subjective_frq_solution" || parsed.detectedContentType === "handwritten_student_work" || parsed.verificationVerdict === "GENUINE_EXAM_ANSWER");
    ...
    if (isAcademicSubjective) {
      ...
      if (!parsed.detectionReason) {
        parsed.detectionReason = parsed.opticalInspection?.verdictReason || (parsed.detectedContentType === "app_logo_or_graphic" ? "The uploaded image contains app logos, graphics, or UI elements rather than academic question or answer materials." : "The image does not contain an authentic academic question or student solution.");
      }
