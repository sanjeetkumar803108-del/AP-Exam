...
4. 💡 **AP Exam Key Takeaway**: A memorable rule or tip to master this for the AP Exam.`
  ...
  const handleNextQuestion = () => {
    ...
    if (currentQuestionIndex + 1 < activeQuizLevel.questions.length) {
      ...
      // Confetti only if user passed with at least 1 correct answer
      if (isPassed) {
        confetti({
          particleCount: 80,
          spread: 70,
  ...
  return (
    <div className="min-h-full flex flex-col bg-[#FAF8F5] light-surface text-zinc-950 relative overflow-hidden font-sans select-none">
      ...
      />
      {/* Top Header Bar */}
      <header className="px-4 sm:px-6 py-3 flex items-center justify-between border-b border-zinc-200/80 bg-white/95 backdrop-blur-md sticky top-0 z-40">
      ...
      <div className="sticky top-14 z-30 pointer-events-none flex justify-center px-4 mb-[-36px]">
        <motion.div
          ...
          className="pointer-events-auto flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/95 backdrop-blur-md border border-zinc-200/90 shadow-md text-zinc-950 transition-all"
        >
      ...
      <main
        ...
        className="flex-1 w-full overflow-y-auto overflow-x-hidden flex flex-col items-center py-8 px-3 relative z-10"
        style={{
      ...
      >
        ...
        >
              ...
              >
                ...
                >
                  ...
                  <div className="absolute top-4 left-4 right-4 flex items-center justify-between pointer-events-auto">
                    <div className="flex items-center gap-2 bg-white/90 backdrop-blur-xs px-3 py-1.5 rounded-2xl border border-zinc-200/80 shadow-xs">
                    ...
                    <div className="bg-white/90 backdrop-blur-xs px-2.5 py-1 rounded-xl border border-zinc-200 text-[10px] font-black text-emerald-800 flex items-center gap-1 shadow-2xs">
          ...
          >
            ...
            <span className="text-[10px] font-black uppercase tracking-wider text-teal-950 bg-teal-50/95 backdrop-blur-xs border border-teal-300/80 px-3.5 py-1.5 rounded-full shadow-sm mt-1">
          ...
          >
            ...
            <span className="text-[11px] font-black uppercase tracking-wider text-purple-950 bg-purple-50/95 backdrop-blur-xs border border-purple-300 px-4 py-1.5 rounded-full shadow-md mt-1 flex items-center gap-1.5">
      ...
      {/* ========================================================================= */}
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-30 max-w-[95vw] sm:max-w-2xl bg-white/95 backdrop-blur-md border border-zinc-300/80 rounded-2xl p-1.5 shadow-[0_10px_25px_rgba(0,0,0,0.15)] flex items-center gap-1.5 overflow-x-auto">
      ...
      <AnimatePresence>
        {activeQuizLevel && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] bg-white light-surface flex flex-col justify-between overflow-hidden"
          >
            {/* Quiz Top Header */}
            <header className="px-4 py-3.5 border-b border-zinc-200 flex items-center justify-between bg-white shrink-0">
              <button
                onClick={() => {
                  triggerVibration(10);
            ...
            {/* Quiz Content Body */}
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 max-w-2xl mx-auto w-full flex flex-col">
              {currentQ && (
            ...
            <footer className="p-4 border-t border-zinc-200 bg-white shrink-0">
              <div className="max-w-2xl mx-auto flex items-center justify-between gap-3">
                <button
                  onClick={() => setActiveQuizLevel(null)}
                  className="px-4 py-2.5 rounded-xl border border-zinc-300 text-xs font-bold text-zinc-700 hover:bg-zinc-100"
                >
                  Quit Level
                </button>
                ...
                {!isAnswerSubmitted ? (
                  <button
                    ...
                    className="flex-1 py-3 rounded-xl bg-amber-500 hover:bg-amber-600 disabled:opacity-40 disabled:cursor-not-allowed text-white font-black text-sm shadow-sm transition-all"
                  >
                    Check Answer
                  </button>
                ) : (
                  <button
                    ...
                    className="flex-1 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-black text-sm shadow-sm transition-all flex items-center justify-center gap-1.5"
                  >
                    <span>
                      {currentQuestionIndex + 1 < activeQuizLevel.questions.length
                        ? 'Next Question'
          ...
          return (
            <div className="fixed inset-0 z-[110] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
              <motion.div
                ...
                className="w-full max-w-sm bg-white rounded-3xl p-6 border border-zinc-200 shadow-2xl text-center relative overflow-hidden light-surface"
              >
                {/* Header Icon Circle */}
