...
import { sharePDFMobile } from '../utils/mobileSaver';
...
export default function QuizGenerator({ onBack }: { onBack: () => void }) {
  ...
  const drawPerformanceCard = (
    ...
    ctx.fillText('AP EXAM APP QUIZ PERFORMANCE', 540, 100);
    ...
    ctx.fillText('Quiz made with AP Exam App', 540, 980);
  ...
  // Generate & Share Complete Quiz Questions, Student Results & Solutions as PDF
  const handleShareQuizPDF = async () => {
    if (!quiz || quiz.length === 0) {
      showToast('No quiz questions to share', 'warning');
      return;
    }
    triggerVibration(15);
    try {
      ...
      const drawHeader = (isFirstPage: boolean) => {
        if (isFirstPage) {
          ...
          doc.setFontSize(8.5);
          doc.text('AP EXAM APP  |  AUTHENTIC AP CURRICULUM QUIZ ASSESSMENT', margin, 24);

          doc.setTextColor(255, 255, 255);
          doc.setFont('helvetica', 'bold');
          doc.setFontSize(15);
          doc.text(`${topic || 'AP Practice Quiz'} - Assessment & Solutions`, margin, 46);

          doc.setFont('helvetica', 'normal');
          doc.setFontSize(8.5);
          doc.setTextColor(203, 213, 225);
          doc.text(`Total Questions: ${quiz.length}  |  Score: ${score}/${quiz.length} (${Math.round((score/quiz.length)*100)}%)  |  Generated by AP Exam App`, margin, 64);
          currentY = 96;
        } else {
          ...
          doc.text(`AP Exam App Quiz: ${topic || 'Practice Assessment'}`, margin, 18);
          currentY = 44;
        }
      };
      ...
      const drawFooter = (pageNum: number) => {
        ...
        doc.text('For step-by-step interactive AI hints & audio tutoring, practice on AP Exam App', margin, pageHeight - 10);
      ...
      doc.text(`Official Assessment Score: ${score} / ${quiz.length} Correct (${Math.round((score/quiz.length)*100)}%)`, margin + 12, currentY + 16);
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(8);
      doc.setTextColor(55, 65, 81);
      doc.text('Review all question prompts, chosen answers, correct answers, and AI pedagogical explanations below.', margin + 12, currentY + 28);
      currentY += 48;
      ...
      quiz.forEach((q, idx) => {
        ...
        doc.roundedRect(margin, currentY, 20, 14, 2, 2, 'F');
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(8.5);
        doc.setTextColor(67, 56, 202);
        doc.text(`Q${idx + 1}`, margin + 3, currentY + 10);

        // Question Prompt
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(9.5);
        doc.setTextColor(15, 23, 42);
        const cleanPrompt = q.question.replace(/\\\[|\\\]|\\\(|\\\)/g, ' ').replace(/\$/g, '').trim();
        const promptLines = doc.splitTextToSize(cleanPrompt, contentWidth - 28);
        let pY = currentY + 10;
        promptLines.forEach((line: string) => {
          checkPageBreak(14);
          doc.text(line, margin + 26, pY);
          pY += 12;
        });
        currentY = pY + 4;
        ...
        if (q.options && q.options.length > 0) {
          q.options.forEach((opt, optIdx) => {
            ...
            const isCorrect = String(q.correctAnswer).trim().toLowerCase() === opt.trim().toLowerCase() || String(q.correctAnswer).trim().toUpperCase() === optLetter;
            
            if (isCorrect) {
              doc.setFillColor(220, 252, 231);
              doc.rect(margin + 6, currentY - 8, contentWidth - 12, 14, 'F');
            ...
            doc.setFontSize(8.5);
            const cleanOpt = opt.replace(/\\\[|\\\]|\\\(|\\\)/g, ' ').replace(/\$/g, '').trim();
            doc.text(`(${optLetter}) ${cleanOpt}${isCorrect ? '  ✓ [CORRECT]' : ''}`, margin + 12, currentY + 2);
            currentY += 15;
          });
        }
        ...
        if (q.explanation) {
          ...
          doc.setFillColor(248, 250, 252);
          doc.setDrawColor(226, 232, 240);
          const cleanExp = q.explanation.replace(/\\\[|\\\]|\\\(|\\\)/g, ' ').replace(/\$/g, '').trim();
          const expLines = doc.splitTextToSize(`Explanation: ${cleanExp}`, contentWidth - 24);
          const expBoxHeight = Math.max(22, expLines.length * 11 + 10);
          
          checkPageBreak(expBoxHeight + 8);
          doc.roundedRect(margin + 6, currentY, contentWidth - 12, expBoxHeight, 3, 3, 'FD');
          doc.setFillColor(99, 102, 241);
          doc.rect(margin + 6, currentY, 2.5, expBoxHeight, 'F');
          ...
          expLines.forEach((line: string) => {
            doc.text(line, margin + 14, eY);
            eY += 11;
          });
          currentY += expBoxHeight + 14;
        }
      ...
      const safeTopic = (topic || 'AP_Quiz').replace(/[^a-zA-Z0-9_-]/g, '_');
      const filename = `${safeTopic}_Quiz_and_Solutions.pdf`;
      const pdfBlob = doc.output('blob');
      await sharePDFMobile(pdfBlob, filename);
    } catch (err: any) {
      console.error('Failed to generate/share quiz PDF:', err);
      showToast('Failed to share quiz PDF: ' + (err.message || err), 'error');
    }
  };
        ...
        {quizState === 'results' && (
          ...
          >
            ...
            <div className="w-full space-y-3">
              <button
                onClick={handleShareQuizPDF}
                className="w-full bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white py-4 rounded-2xl font-bold text-sm transition-all active:scale-98 border border-emerald-500/30 flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/10 cursor-pointer"
              >
                <Share2 className="w-4 h-4" />
                <span>Share Quiz & Solutions (PDF)</span>
              </button>

              <button
                onClick={handleOpenShareModal}
                className="w-full bg-zinc-100 hover:bg-zinc-200 text-zinc-800 py-3.5 rounded-2xl font-bold text-xs transition-all active:scale-98 border border-zinc-200 flex items-center justify-center gap-2 cursor-pointer"
              >
            ...
            >
              ...
              <div className="flex flex-col items-center text-center">
                ...
                <div className="w-full mb-4">
                  <button
                    onClick={handleShareQuizPDF}
                    className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-bold py-3.5 px-4 rounded-2xl text-xs flex items-center justify-center gap-2 transition-all active:scale-98 shadow-lg shadow-emerald-600/15 cursor-pointer mb-2.5"
                  >
                    <Share2 className="w-4 h-4" />
                    <span>Share Full Quiz Document (PDF)</span>
                  </button>
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const file = new File([blob], 'AP_Exam_Quiz_Score.png', { type: 'image/png' });
                        
                        if (navigator.canShare && navigator.canShare({ files: [file] })) {
                          await navigator.share({
                            files: [file],
                            title: 'My AP Exam Quiz Score!',
                            text: `Check out my score on AP Exam App! 🧠 I scored ${score}/${quiz.length} (${Math.round((score/quiz.length)*100)}%) on ${topic || 'Custom Assessment'}.`,
                          });
